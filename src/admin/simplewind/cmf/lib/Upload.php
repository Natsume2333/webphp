...
class Upload
{
    ...
    public function upload()
    {
      ...
      //  var_dump(config('FILE_UPLOAD_TYPE'));
        $arrInfo = [];
        $storage = cmf_get_option('storage');   //判断是否是七牛储存
            ...
            if ($storage['type'] != 'Local') {
                ...
                $name=$this->qiniu_Upload($arrInfo);
                return [
                    'filepath'    => "http://".$name['filepath'],
                    "name"        => $name['name'],
                    'id'          => $strId,
                    'preview_url' => "http://".$name['filepath'],
                    'url'         =>"http://".$name['filepath'],
                ];
     ...
     //   var_dump($arrInfo);
        $needUploadToRemoteStorage = true;//是否要上传到云存储
        if ($objAsset) {
...
//        }
    ...
    //七牛上传 $file文件
//    function qiniu_Upload($strWebPath,$fileSaveName){
//        // $filePath=$file['tmp_name'];
//        // 上传到七牛后保存的文件名
//        $filePath="upload/".$strWebPath.$fileSaveName;
//
//        // 要上传图片的本地路径
...
//            $arr='';
//        }else{
//            $arr=array('filepath'=>$domain .'/'. $ret['key'],'name'=>$ret['key']);
//        }
//
//        return $arr;
//    }
    ...
    function qiniu_Upload($file){
        ...
        // 要上传图片的本地路径
   // var_dump($file);
        $filePath="upload/".$file['file_path'];
