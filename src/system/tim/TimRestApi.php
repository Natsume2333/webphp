...
{
	#app基本信息
	protected $sdkappid = 1400286709;
	public $usersig = '';
	protected $identifier = '';
    protected $tencent_sha_key = '';
	 ...
	 */
	function init($sdkappid, $identifier,$tencent_sha_key = 0)
	{
	
		$this->sdkappid = $sdkappid;
		$this->identifier = $identifier;
        $this->tencent_sha_key = $tencent_sha_key;
	}
...
//			return null;
//		}
//		$out[0] = file_get_contents("http://vlapi.bugukj.com/get_sign.php?user_id=".$identifier);
//		//echo $sign;
//		//print_r($command);exit;
//		$this->usersig = $out[0];

        require_once(DOCUMENT_ROOT . '/system/tim/TLSSigAPIv2.php');
        // require_once "../src/TLSSigAPIv2.php"; // 源码集成使用相对路径

        // $api = new \Tencent\TLSSigAPIv2($this->sdkappid, $this->tencent_sha_key);
		//$api = new \Tencent\TLSSigAPIv2("1400290962","f270fc576e30e84465fe2f2e84c36b0e33b7fcdb83595c2fec8653a76102955a");
		$api = new \Tencent\TLSSigAPIv2("1400240038","bf97a2906e17a9d2935bebf444be7098122655b69b604bf2b2e0bdc7ff14dfc3");
        $out[0] = $api->genSig($identifier);
		return $out;
	}
    ...
    function kick($account_id)
    {
        ...
        $ret = $this->api("im_open_login_svc", "kick", $this->identifier, $this->usersig, $req_data);
        $ret = json_decode($ret, true);
        return $ret;
    }

    //全局禁言
    function shut_up($account_id,$time)
    {
        #构造新消息
        $msg = array(
        ...
        $ret = $this->api("group_open_http_svc", "setnospeaking", $this->identifier, $this->usersig, $req_data);
        $ret = json_decode($ret, true);
        return $ret;
    ...
    //查询用户在线状态
    function check_online_status($account_id)
    {
        $msg = ["To_Account" => [$account_id]];
        #将消息序列化为json串
        $req_data = json_encode($msg);
        $ret = $this->api("openim", "querystate", $this->identifier, $this->usersig, $req_data);
        $ret = json_decode($ret, true);
        return $ret;
