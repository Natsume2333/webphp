...
class Captcha
{
     ...
     * @return bool 用户验证码是否正确
     */
    public function check_redis($code, $id = '')
    {
        $key = $GLOBALS['redis'] -> get('img_code:key:'.$id);
        // 验证码不能为空
        $secode = $GLOBALS['redis'] -> get('img_code:'.$id);
        if (empty($code) || empty($secode)) {
            return false;
        }
        // session 过期
...
//        }
        ...
        if ($this->authcode(strtoupper($code)) == $secode) {
            $this->reset && $GLOBALS['redis'] -> del('del',$id);
            return true;
        }
    ...
    public function entry($id = '')
    {
        ...
        $GLOBALS['redis'] -> set('img_code:'.$id,$code,60);
        $GLOBALS['redis'] -> set('img_code:key:'.$id,$key,60);
        ob_start();
        // 输出图像
